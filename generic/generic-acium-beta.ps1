...
<#
CHANGELOG — generic-acium-beta.ps1 vs. generic-acium.ps1
=============================================================================
generic-acium.ps1 is the stable script. This file is a hardened revision
of it, kept as a SEPARATE file rather than edited in place so that:
  * anything already running generic-acium.ps1 (a scheduled task, another
    RMM's script slot) keeps behaving exactly as it does today — nothing
    changes until someone deliberately swaps in this file instead;
  * this revision can be rolled out to a small subset of endpoints and
    run there for a while before being promoted more broadly;
...
replace generic-acium.ps1 and this file can be retired.

Changes in this beta (vs. generic-acium.ps1), and why each was made:
  ...
  * Downloads retry, use the system proxy, and MSI property values are
    quoted correctly (an $Organization containing a space silently
    broke the old argument-array approach).
#>
...
.SYNOPSIS
    ...
    BETA. This is a hardened revision of generic-acium.ps1. See the
...
.NOTES
    ...
    changes in this beta (vs. generic-acium.ps1), why each one was made,
    and why this revision lives in its own file instead of replacing
    generic-acium.ps1 directly.
