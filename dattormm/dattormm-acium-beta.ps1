...
<#
CHANGELOG — dattormm-acium-beta.ps1 vs. dattormm-acium.ps1
=============================================================================
dattormm-acium.ps1 is the stable script, currently deployed fleet-wide.
This file is a hardened revision of it, kept as a SEPARATE file rather
than edited in place so that:
  * the stable script keeps behaving exactly as it does today for every
    Datto RMM Component already pointed at it — nothing changes until
    someone deliberately re-points a Component at this file instead;
  * this revision can be assigned to a small test Job/device group and
    run there for a while before being promoted fleet-wide;
  * a plain file diff between the two IS the full list of changes, which
    is what the list below is generated from.
Once this revision has been validated in production, its contents should
replace dattormm-acium.ps1 and this file can be retired.

Changes in this beta (vs. dattormm-acium.ps1), and why each was made:

  * ORGANIZATION support, via the new AgentOrganization Component
    Variable. The previous Datto script had no way to set it at all, so
    Datto-deployed sensors installed with no organization ID while the
    generic script set one — a silent divergence between the two
    copies. Datto is the platform WITH a variable system, so this is
    the one place a per-site org ID should be trivial.
  * ProductCode self-check. The hardcoded $ProductCode is now
    cross-checked against the Uninstall registry. If Windows Installer
    says "not installed" but a matching product IS registered, the
    script logs the REAL ProductCode and uses it, instead of silently
    taking the first-time-install branch on an existing install (which
    is what produces error 1638). See VERIFY THE PRODUCTCODE in .NOTES.
  * Reboot gate. If the ASP.NET Core hosting bundle install returns
    3010 (reboot required), the script now STOPS instead of running the
    sensor MSI whose service cannot yet start — that ordering was
    reproducing the exact 1603/1920 failure the prereq check exists to
    prevent. The next scheduled run completes the install.
  * SHA256 change detection in addition to ETag. If the server ever
    stops returning an ETag header, the old script would reinstall the
    MSI on every scheduled run, fleet-wide, forever — and report
    success every time. The package hash now catches that.
  * Install state is read from the service, not a running process. A
    stopped-but-installed service no longer looks like "not installed"
    and trigger a reinstall on every run.
  * $WorkDir/$LogDir ACLs are hardened. C:\ProgramData's default DACL
    lets standard users create directories in inherited subfolders;
    this script stages an MSI there and executes it as SYSTEM.
  * Authenticode check on the MSI before running it as SYSTEM.
  * Write-Log can no longer kill the script. Under
    $ErrorActionPreference='Stop', a locked log file was an unhandled
    terminating error — exiting 1, which the exit-code table reads as
    "download failed."
  * msiexec 1618 ("another install in progress") is retried.
  * TLS is OR'd into the existing protocol set rather than replacing
    it, so TLS 1.3 isn't disabled on newer OSes.
  * Log files rotate instead of growing unbounded; the MSI log appends
    instead of truncating every run.
  * Downloads retry, use the system proxy, and MSI property values are
    quoted correctly (an AgentOrganization containing a space would
    otherwise silently break the argument-array approach).
#>
...
.SYNOPSIS
    ...
    BETA. This is a hardened revision of dattormm-acium.ps1. See the
    CHANGELOG at the top of this file before deploying it broadly.
...
.NOTES
    ...
    See the CHANGELOG at the very top of this file for the full list of
    changes in this beta (vs. dattormm-acium.ps1), why each one was made,
    and why this revision lives in its own file instead of replacing
    dattormm-acium.ps1 directly.
